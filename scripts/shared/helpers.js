...
const providers = {
  ...
  skale: new ethers.providers.JsonRpcProvider(SKALE_URL)
}
...
const signers = {
  ...
  skale: new ethers.Wallet(SKALE_DEPLOY_KEY).connect(providers.skale)
}
...
async function getFrameSigner() {
  try {
    const frame = new ethers.providers.JsonRpcProvider("https://skale.crypto.org:8545")
...
async function deployContract(name, args, label, options, registerName) {
  ...
  syncDeployInfo(registerName ?? name, {
    ...
    params: `${contract.address} ${argStr}`
  });
