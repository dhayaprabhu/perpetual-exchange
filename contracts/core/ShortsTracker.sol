...
contract ShortsTracker is Governable, IShortsTracker {
    ...
    function getNextGlobalShortData(
        ...
        // avoid stack to deep
        {
            uint256 size = vault.globalShortSizes(_collateralToken);
    ...
    function getRealisedPnl(
        ...
        IVault _vault = vault;
        ( IVault.Position memory position, , ) = _vault.getPosition(_account, _collateralToken, _indexToken, false);

        (bool hasProfit, uint256 delta) = _vault.getDelta(_indexToken, position.size, position.averagePrice, false, position.lastIncreasedTime);
