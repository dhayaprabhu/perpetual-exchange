...
contract Reader is Governable {
    ...
    uint256 public constant POSITION_PROPS_LENGTH = 11;
    ...
    function getFundingRates(address _vault, address[] memory _tokens) public view returns (uint256[] memory) {
        uint256 propsLength = 2;
    ...
    function getVaultTokenInfo(address _vault, uint256 _usdgAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getFullVaultTokenInfo(address _vault, uint256 _usdgAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getVaultTokenInfoV2(address _vault, uint256 _usdgAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getPositions(address _vault, address _account, address[] memory _collateralTokens, address[] memory _indexTokens, bool[] memory _isLong) public view returns(uint256[] memory) {
        ...
        for (uint256 i = 0; i < _collateralTokens.length; i++) {
            ...
            amounts[i * POSITION_PROPS_LENGTH + 1] = position.collateral;
            amounts[i * POSITION_PROPS_LENGTH + 2] = position.averagePrice;
            amounts[i * POSITION_PROPS_LENGTH + 3] = position.entryFundingRate;
            amounts[i * POSITION_PROPS_LENGTH + 4] = hasRealisedProfit ? 1 : 0;
            amounts[i * POSITION_PROPS_LENGTH + 5] = realisedPnl;
            amounts[i * POSITION_PROPS_LENGTH + 6] = position.lastIncreasedTime;
            amounts[i * POSITION_PROPS_LENGTH + 9] = position.slPercent;
            amounts[i * POSITION_PROPS_LENGTH + 10] = position.tpPercent;
            }
